...
def test_example_NN():
    ...
    options = {"numepochs":2,"batchsize":100,"plot":1}
    nn,L = nntrain.nntrain(nn,train_x,train_y,options,1)
    ...
    nn,L = nntrain.nntrain(nn,train_x,train_y,options,2)
    er,bad = nntest.nntest(nn,test_x,test_y)
    assert (er < 0.08),"Too big error"
    del nn
    ...
    nn,L = nntrain.nntrain(nn,train_x,train_y,options,3)
    er,bad = nntest.nntest(nn,test_x,test_y)
    ...
    nn.LearningRate = 1
    nn,L = nntrain.nntrain(nn,train_x,train_y,options,4)
    er,bad = nntest.nntest(nn,test_x,test_y)
    assert (er < 0.1),"Too big error"
    del nn    
    ...
    nn,L = nntrain.nntrain(nn,train_x,train_y,options,5)
    er,bad = nntest.nntest(nn,test_x,test_y)
    assert (er < 0.1),"Too big error"
    del nn
